...
function atmos_init_aux!(::GCMInput, ::AtmosModel, aux::Vars, geom::LocalGeometry) end
function atmos_nodal_update_aux!(::GCMInput, ::AtmosModel, state::Vars, aux::Vars, t::Real) end
...
function flux_radiation!(::GCMInput, atmos::AtmosModel, flux::Grad, state::Vars, aux::Vars, t::Real) end

# --------------------------- Do nothing --------------------------------------- # 
struct NoCFSites <: GCMInput end #

struct CFSource <: Source
end

function atmos_source!(::CFSource, atmos::AtmosModel, source::Vars, state::Vars, aux::Vars, t::Real)
...
"""
struct GCMForcedState <: ReferenceState
end
vars_aux(m::GCMForcedState, FT) = @vars(ρ::FT, p::FT, T::FT, ρu::SVector{3,FT}, ρe::FT, ρq_tot::FT)
# Idea is to pass additional args and store them in here for access through the source function 
function atmos_init_aux!(m::GCMForcedState, atmos::AtmosModel, aux::Vars, geom::LocalGeometry, splines)
  FT = eltype(aux)
